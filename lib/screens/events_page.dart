...
class EventsPage extends StatefulWidget {
  ...
  const EventsPage({super.key, required this.userEmail});
...
class _EventsPageState extends State<EventsPage> {
  ...
  Future<void> _loadEvents() async {
    ...
    try {
      final events = await _db.getEvents(widget.userEmail);
      setState(() {
        _events = events;
        _isLoading = false;
      });
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  ListTile(
                    title: Text(date != null ? DateFormat('d MMM yyyy').format(date!) : 'Date (Optional)'),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  if (existing != null) {
                    final ok = await _db.updateEvent(widget.userEmail, event);
                    if (ok && mounted) {
                      setState(() {
                        final i = _events.indexWhere((e) => e.id == existing.id);
                    ...
                    final id = await _db.addEvent(widget.userEmail, event);
                    if (id != null && mounted) {
                      setState(() => _events.add(event.copyWith(id: id)));
                      try {
                         ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Event added'), backgroundColor: Colors.green),
                      );
                      } catch (e) {
  ...
  Future<void> _deleteEvent(Event event) async {
    ...
    final ok = await _db.deleteEvent(widget.userEmail, event.id);
    if (ok && mounted) {
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              if (event.date != null || event.startDate != null) ...[
